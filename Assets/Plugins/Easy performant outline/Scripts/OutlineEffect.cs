...
namespace EPOOutline
{
    public static class OutlineEffect
    {
        ...
        public static void SetupOutline(OutlineParameters parameters)
        {
            ...
            drawnOutlinablesCount += DrawOutlineables(parameters, CompareFunction.Always,    false, 0.0f, x => x.OutlineParameters.Enabled, x => x.OutlineParameters.Color, OutlineMaterial, RenderStyle.Single);
            drawnOutlinablesCount += DrawOutlineables(parameters, CompareFunction.NotEqual,  false, 0.0f, x => x.BackParameters.Enabled,    x => x.BackParameters.Color, OutlineMaterial, RenderStyle.FrontBack);
            drawnOutlinablesCount += DrawOutlineables(parameters, CompareFunction.LessEqual, false, 0.0f, x => x.FrontParameters.Enabled,   x => x.FrontParameters.Color, OutlineMaterial, RenderStyle.FrontBack);
            ...
            foreach (var outlinable in parameters.OutlinablesToRender)
            {
                if (outlinable.RenderStyle == RenderStyle.FrontBack)
                {
                    ...
                    foreach (var target in outlinable.OutlineTargets)
                    {
                        ...
                        var shouldDraw = outlinable.FrontParameters.Enabled && outlinable.FrontParameters.FillPass.Material != null;
                        parameters.Buffer.DrawRenderer(renderer, shouldDraw ?
                            outlinable.FrontParameters.FillPass.Material :
                            EmptyFillMaterial, target.ShiftedSubmeshIndex);
                    }
                    ...
                    foreach (var target in outlinable.OutlineTargets)
                    {
                        ...
                        parameters.Buffer.DrawRenderer(renderer, outlinable.BackParameters.FillPass.Material, target.ShiftedSubmeshIndex);
                    }
                }
                else
                {
                    ...
                    foreach (var target in outlinable.OutlineTargets)
                    {
                        ...
                        parameters.Buffer.DrawRenderer(renderer, outlinable.OutlineParameters.FillPass.Material, target.ShiftedSubmeshIndex);
                    }
                }
            }
        }
        ...
        private static void SetupCutout(OutlineParameters parameters, OutlineTarget target)
        {
                ...
                materialToGetTextureFrom.HasProperty(target.CutoutTextureId))
            {
                ...
                parameters.Buffer.SetGlobalTexture(CutoutTextureHash, materialToGetTextureFrom.GetTexture(target.CutoutTextureId));
            }
            else
                parameters.Buffer.DisableShaderKeyword(KeywordsUtility.GetCutoutKeyword());
